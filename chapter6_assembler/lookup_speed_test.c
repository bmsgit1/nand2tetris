...
comp_entry *linear_search(comp_entry *comp, int num_keys, char *key);
comp_entry *binary_search(comp_entry *comp, int num_keys, char *key);
    ...
    for (int num_keys=1; num_keys<29; num_keys++) {
        ...
        for (int key_index=0; key_index<num_keys; key_index++) {
            ...
            for (int run=0; run<num_runs; run++) {
                found_entry = linear_search(comp_table, num_keys, key);
            }
        }
        clock_t end = clock();
        ...
        for (int item_index=0; item_index<num_keys; item_index++) {
            ...
            for (int run=0; run<num_runs; run++) {
                found_entry = binary_search(comp_table, num_keys, key);
            }
        }
        end = clock();
